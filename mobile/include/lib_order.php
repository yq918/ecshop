...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $plus_price = get_final_price($goods_id, $num, true, $spec, true);
                $goods_price = get_final_price($goods_id, $num, true, $spec);
        ...
        else //购物车没有此物品，则插入
        {
        	$plus_price = get_final_price($goods_id, $num, true, $spec, true);
            $goods_price = get_final_price($goods_id, $num, true, $spec);
            $parent['plus_price']  = max($plus_price, 0);
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['plus_reduce'] = $row['plus_price']?$row['plus_price']-$row['goods_price']:0;
		$row['goods_price_noformat'] = $row['goods_price'];
		$row['market_price_noformat'] = $row['market_price'];
		$row['plus_price_noformat'] = $row['plus_price'];
