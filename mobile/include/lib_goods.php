...
function get_top10($cats = '')
{
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        ...
        $arr[$i]['price'] = price_format(plus_price($arr[$i]['shop_price'], $arr[$i]['goods_id']));
    }
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['shop_price']   = price_format(plus_price($row['shop_price'],  $row['goods_id']));
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $row['shop_price_formated'] = price_format(plus_price($row['shop_price'], $goods_id));
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            foreach ($res AS $row)
            {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['shop_price']    = price_format(plus_price($row['shop_price'], $row['goods_id']));
...
function get_goods_fittings($goods_list = array())
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$temp_index]['shop_price']        = price_format(plus_price($row['shop_price'], $row['goods_id']));//配件原价格
