...
$smarty->assign('request_page', 'user.php');
...

{
    ...
    if (!empty($bank_account) && !preg_match('/^\d+$/', $bank_account))

    {

         show_message("银行账号格式不对");
    ...
    if ($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $profile, 'UPDATE', "user_id = '$profile[user_id]'"))
...

{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($user_id>0 && $user_id == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    ...
    if(is_array($orders)){
        ...
        foreach($orders as $vo){
            ...
            <tr>
                ...
                <td><img src="'.$config['site_url'].'mobile/'.$img.'" width="50" height="50" /></td>
...
elseif ($action == 'service_add' ){
	 ...
	 $service_sn    = "S" . time() . rand(10000,9999);

	 $sql = "INSERT INTO " .$ecs->table('service')."(order_id, user_id, service_sn, service_type, consignee, tel, question_desc, service_desc, add_time) VALUES ('$order_id', '$user_id', '$service_sn', '$service_type', '$consignee', '$tel', '$question_desc', '$service_desc', ".time().")";

	 $db->query($sql);

	 show_message('申请成功，等待客服反馈', '返回用户中心', 'user.php');	 
...

{
    ...
    if (count($consignee_list) < 5 && $user_id > 0)
...
elseif ($action == 'async_collection_list'){
    ...
    $collections = get_collection_goods($user_id, $limit=10, $start=0);
...

{
	...
	$profile = $db->getRow("SELECT * FROM  " .$ecs->table('users').
...

{
...

    {
		...
		if($amount<$wxconf['withdraw_lower_limit'])

	    {

           show_message("申请提现不能低于".$wxconf['withdraw_lower_limit']."元");
...

    {
		...
		if($amount<$wxconf['deposit_lower_limit'])

	    {

           show_message("充值不能低于".$wxconf['deposit_lower_limit']."元");
		...
		$user_info = $db->getRow("SELECT user_name,nick_name FROM  " . $ecs->table('users') . " WHERE user_id=$user_id");

        $surplus['admin_note'] = "[" . $user_info[user_name] . "]" . $user_info[nick_name] . " 充值".$amount."元";
		...
		$sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('pay_log')." (order_id, order_amount, order_type, is_paid, user_account_id)".

            " VALUES  ('$new_order_id', '$amount', '$type', '$is_paid', '$surplus[rec_id]')";
...
{	
	...
	$sql = "SELECT * FROM " . $ecs->table('account_log') . " WHERE user_id = ".$user_id;
