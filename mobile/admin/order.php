...
elseif($_REQUEST['act'] == 'affiliate_plus') {//打赏粉丝
	...
	if ($order_id){
		...
		//查询订单信息
		$sql = 'SELECT og.goods_id,og.goods_number,og.goods_price,oi.user_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' og left join ' . $GLOBALS['ecs']->table('order_info') . ' oi on (og.order_id=oi.order_id) WHERE og.order_id =  ' . $order_id;
		$result = $GLOBALS['db']->getAll($sql);

		$affiliate_price=$final_price=0;
		foreach ($result as $goods) {
			...
			if ($all_affiliate_persons>=SUPPER_PRICE_NUM){
				$affiliate_price = $goods_plus[4]*$percent_jiajia*(1-$percent_youhui);
			} else if ($all_affiliate_persons>=MIDDLE_PRICE_NUM) {
				$affiliate_price = $goods_plus[3]*$percent_jiajia*(1-$percent_youhui);
			} else {
				$affiliate_price = $goods_plus[2]*$percent_jiajia*(1-$percent_youhui);
			}
			//计算打赏金额
			$final_price += $affiliate_price;
			//echo $goods_id,',',$affiliate_price,'<br>';
		...
		$smarty->assign('user', $user);
		$smarty->assign('all_affiliate_persons',  $all_affiliate_persons);
		$smarty->assign('final_price',  $final_price);

		//查询上级粉丝
		$nums=0;
		$users=$users_id_arr=array();
		if ($final_price) {
			$affiliate_persons = $GLOBALS['db']->getAll("SELECT affiliate_id,second_affiliate_id,third_affiliate_id FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id=".$user_id);
			//
			$affiliate_id = $affiliate_persons[0]['affiliate_id'];
			$second_affiliate_id = $affiliate_persons[0]['second_affiliate_id'];
			$third_affiliate_id = $affiliate_persons[0]['third_affiliate_id'];

			if ($affiliate_id) {
				$nums+=1;
				$users_id_arr[]=$affiliate_id;
				$sql = 'SELECT user_name FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id =  ' . $affiliate_id;
				$users['affiliate_id'] = $GLOBALS['db']->getOne($sql);
			}
			if ($second_affiliate_id) {
				$nums+=1;
				$users_id_arr[]=$second_affiliate_id;
				$sql = 'SELECT user_name FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id =  ' . $second_affiliate_id;
				$users['second_affiliate_id'] = $GLOBALS['db']->getOne($sql);
			}
			if ($third_affiliate_id) {
				$nums+=1;
				$users_id_arr[]=$third_affiliate_id;
				$sql = 'SELECT user_name FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id =  ' . $third_affiliate_id;
				$users['third_affiliate_id'] = $GLOBALS['db']->getOne($sql);
			}
			//有几个人分几个人，每人金额不增加
			if ($nums){
				//$per_avg = intval($final_price/$nums);
				$per_avg = intval($final_price/3);
			}
			//print_r($per_avg);
		}
		$smarty->assign('nums',  $nums);
		$smarty->assign('users',  $users);
		$smarty->assign('users_id_arr',  $users_id_arr);
		$smarty->assign('per_avg',  $per_avg);
	...
	/* 模板赋值 */
	$smarty->assign('ur_here', '打赏粉丝');
	$smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['02_order_list']));

	/* 显示模板 */
	assign_query_info();
	$smarty->display('affiliate_plus.htm');
}
elseif($_REQUEST['act'] == 'affiliate_plus_insert') {//打赏粉丝

	//
	$user = $_REQUEST['user'];
	$nums = $_REQUEST['nums'];
	$per_avg = $_REQUEST['per_avg'];
	$affiliate_id = $_REQUEST['affiliate_id'];
	$second_affiliate_id = $_REQUEST['second_affiliate_id'];
	$third_affiliate_id = $_REQUEST['third_affiliate_id'];

	if ($nums && $per_avg){
		//
		if ($affiliate_id){
			$sql = "update " . $GLOBALS['ecs']->table('users') . " set user_money= user_money+$per_avg WHERE user_id =  " . $affiliate_id;
			$GLOBALS['db']->query($sql);
			$sql = "insert into " . $GLOBALS['ecs']->table('account_log') . "(user_id,user_money,change_time,change_desc,change_type) values($affiliate_id,$per_avg, '".time()."', '粉丝 $user 订单返现', 2)";
			$GLOBALS['db']->query($sql);
		}
		if ($second_affiliate_id){
			$sql = "update " . $GLOBALS['ecs']->table('users') . " set user_money= user_money+$per_avg WHERE user_id =  " . $second_affiliate_id;
			$GLOBALS['db']->query($sql);
			$sql = "insert into " . $GLOBALS['ecs']->table('account_log') . "(user_id,user_money,change_time,change_desc,change_type) values($second_affiliate_id,$per_avg, '".time()."', '粉丝 $user 订单返现', 2)";
			$GLOBALS['db']->query($sql);
		}
		if ($third_affiliate_id){
			$sql = "update " . $GLOBALS['ecs']->table('users') . " set user_money= user_money+$per_avg WHERE user_id =  " . $third_affiliate_id;
			$GLOBALS['db']->query($sql);
			$sql = "insert into " . $GLOBALS['ecs']->table('account_log') . "(user_id,user_money,change_time,change_desc,change_type) values($third_affiliate_id,$per_avg, '".time()."', '粉丝 $user 订单返现', 2)";
			$GLOBALS['db']->query($sql);
		}
	}
	$links[] = array('text' => '查看订单', 'href' => 'order.php?act=list');
