...

{
...

	{
		...
		$one_level_member_name   = trim($_POST['one_level_member_name']);

		$two_level_member_name   = trim($_POST['two_level_member_name']);

		$three_level_member_name = trim($_POST['three_level_member_name']);

		$init_subscribe_user_counts = $_POST['init_subscribe_user_counts'] ? intval($_POST['init_subscribe_user_counts']) : 0;

		$be_distributor          = $_POST['be_distributor'] ? round($_POST['be_distributor'],2) : 100;
		...
		$is_userdefine_subscribe = intval($_POST['is_userdefine_subscribe']);

		$is_auto_distribute      = intval($_POST['is_auto_distribute']);

		$is_distributor_limit    = intval($_POST['is_distributor_limit']);

		$is_my_distribute_view   = intval($_POST['is_my_distribute_view']);

		$is_my_subscribe_view    = intval($_POST['is_my_subscribe_view']);

		$withdraw_lower_limit    = $_POST['withdraw_lower_limit'] ? round($_POST['withdraw_lower_limit'],2) : 0;

		$deposit_lower_limit     = $_POST['deposit_lower_limit'] ? round($_POST['deposit_lower_limit'],2) : 0;

		$subscribe_guide_page    = trim($_POST['subscribe_guide_page']);

		$distribute_keywords     = trim($_POST['distribute_keywords']);

		$withdraw_alert_msg      = trim($_POST['withdraw_alert_msg']);

		$home_page_style         = intval($_POST['home_page_style']);

		$ret = $db->query("UPDATE `wxch_config` SET `token`='$token',`appid`='$appid',`appsecret`='$appsecret', ".

		   "app_name = '$app_name',  ".

		   "distributor_name = '$distributor_name', ".

		   "one_level_member_name = '$one_level_member_name', ".

		   "two_level_member_name = '$two_level_member_name', ".

		   "three_level_member_name = '$three_level_member_name', ".

		   "init_subscribe_user_counts = $init_subscribe_user_counts, ".
		   ...
		   "is_distributor_limit = $is_distributor_limit, ".

		   "three_level_deduct = $three_level_deduct,".

		   "is_userdefine_subscribe = $is_userdefine_subscribe, ".     

		   "is_auto_distribute = $is_auto_distribute, ".

		   "withdraw_lower_limit = $withdraw_lower_limit, ".  

		   "subscribe_guide_page = '$subscribe_guide_page', ".

		   "deposit_lower_limit = $deposit_lower_limit, ".

		   "is_my_distribute_view = $is_my_distribute_view, ".

		   "is_my_subscribe_view = $is_my_subscribe_view, ".

		   "distribute_keywords = '$distribute_keywords', ".

		   "withdraw_alert_msg = '$withdraw_alert_msg', ".
