...
class wechatCallbackapi {
	...
	public function responseMsg($db, $user, $base_url) {
		...
		require(dirname(__FILE__) . '/../mobile/include/cls_wechat.php');

		require(dirname(__FILE__) . '/../mobile/include/cls_distribute.php');

		require(dirname(__FILE__) . '/../mobile/include/cls_image.php');
		...
		if (!empty($postStr) or $debug == 1) {
			...
			$user_name=$db -> getRow("SELECT `user_name`,nick_name,user_id,is_subscribe FROM `$thistable` WHERE `wxid` ='$fromUsername'");
			...
			if (empty($user_name)) {
				...
				if (!empty($fromUsername)) {
					...
					$user_id = $wechat->create_wechat_user($fromUsername);

				} 

			} else {

				   $wechat->update_userinfo($fromUsername);
...
( '$fromUsername', '$keyword', $time);");
			...
			if ($postObj -> Event == 'subscribe') {
				...
				if (strstr($postObj -> EventKey, 'qrscene_')) {
					...
					$qr_id = $db->getOne("SELECT qid FROM wxch_qr WHERE `type` = 'qr' AND action_name='QR_LIMIT_SCENE' AND `scene`= '".$scene_id."' ORDER BY qid DESC LIMIT 1");

					if($qr_id){

					   $db -> query("UPDATE `wxch_qr` SET `subscribe`=`subscribe` + 1 WHERE qid=".$qr_id);

					   $db -> query("INSERT INTO `wxch_promote_log`(promoter,visitor,visit_time,is_subscribe,subscribe_time) VALUES (".intval($scene_id).", ".$user_id.", ".time().", 1, ".time().")");
					...
					$distribute->affiliate_bound($user_id);
				  ...
				  $is_userdefine_subscribe = $db->getOne("SELECT is_userdefine_subscribe FROM wxch_config");

				  if($is_userdefine_subscribe)

				     $postObj -> EventKey = 'subscribe';

				  else

				     $postObj -> EventKey = 'subdefault';
				...
				$db -> query("UPDATE  $thistable SET  `is_subscribe` = 0,subscribe_time=0  WHERE  `wxid` = '$fromUsername';");
				...
				$update_sql = "UPDATE `wxch_qr` SET `scan`=`scan` + 1 WHERE `type` = 'qr' AND action_name='QR_LIMIT_SCENE' and `scene`= '$scene_id'";

				$db -> query("$update_sql");

				

				$scan_ret = $db -> getRow("SELECT * FROM `wxch_qr` WHERE scene=$scene_id");

				if (!empty($scan_ret)) {

					$msgType = "text";

					$contentStr = "您已经关注过我们了";
...
( '$fromUsername', '$menu_message', $time);");
			...
			if($postObj -> MsgType == 'location'){
			    ...
			    $keyword = 'find_svcenter';

				$menu_message = 'locatoion:' . $find_svcenter . ' Location_X:' . $postObj->Location_X . ' Location_Y:' . $postObj->Location_Y;
...
( '$fromUsername', '$menu_message', $time);");	
			...
			//查找附近服务中心
			...
			}elseif ($keyword == 'find_svcenter'){

			    if(!empty($postObj->Location_X) && !empty($postObj->Location_Y)){

					$lat = $postObj->Location_X;

					$lon = $postObj->Location_Y;

					$sql = 'select * from '.$db -> prefix . 'svcenter where ' .

                           'xpoint > '.$lat.'-1 and xpoint < '.$lat.'+1 and ' .

                           'ypoint > '.$lon.'-1 and ypoint < '.$lon.'+1 '.

                           'order by ACOS(SIN(('.$lat.' * 3.1415) / 180 ) *SIN((xpoint * 3.1415) / 180 ) +COS(('.$lat.' * 3.1415) / 180 ) * COS((xpoint * 3.1415) / 180 ) *COS(('.$lon.'* 3.1415) / 180 - (ypoint * 3.1415) / 180 ) ) * 6380 asc limit 10';

				    $svcenter = $db->getAll($sql);
				...
				if($is_find){
					...
					$ArticleCount = count($svcenter);

					foreach($svcenter as $k=>$v) {

						if(empty($v['sv_img'])){

						  $url = $_SERVER['HTTP_HOST'];	

						  $svcenter_pic = "http://" . $url . "/images/location.png";

						}else 

						  $svcenter_pic = $base_img_path . $v['sv_img'];

                        $svcenter_url = $m_url . 'svcenter.php?act=detail&id=' . $v['sv_id'];

						$items .= "<item>

                               <Title><![CDATA[" . $v['sv_name'] . " \r\n电话：" . $v['tel'] . " \r\n地址： " . $v['address'] . "]]></Title>

                               <PicUrl><![CDATA[" . $svcenter_pic . "]]></PicUrl>

                               <Url><![CDATA[" . $svcenter_url . "]]></Url>
					...
					$contentStr = "附近没有找到服务中心，<a href=\"".$m_url."svcenter.php\">点击关键词查找</a>";
...
nation=大雁塔&mode=driving&region=西安';
			...
			} elseif ($keyword == 'subdefault' || $keyword == 'qr') {

				$wxconf = $db->getRow("SELECT app_name,init_subscribe_user_counts FROM wxch_config");

				$appname = $wxconf['app_name'];

				$init_counts = $wxconf['init_subscribe_user_counts'];

				$msgType = "text";

				$usertb = $db -> prefix . 'users';

				$userinfo = $db->getRow("SELECT affiliate_id,user_id,user_name FROM $usertb WHERE user_id=".$user_id."");

				$user_counts = $db->getOne("SELECT COUNT(*) FROM $usertb WHERE user_id<".$userinfo[user_id]."");

				$user_counts += $init_counts;

				if($userinfo[affiliate_id]>0){

				   $tj_person = $db->getOne("SELECT nick_name FROM $usertb WHERE user_id=".$userinfo[affiliate_id]);

				   if($tj_person){

				       $rmsg = "您已通过好友【".$tj_person."】的推荐成功关注".$appname."官方微信！您是第".$user_counts."会员，会员ID为【".$userinfo[user_name]."】，点击左下角的菜单，展开".$appname."神秘面纱";

				   }

				   else{

					   $rmsg = "您已成功关注".$appname."官方微信！您是第".$user_counts."会员，会员ID为【".$userinfo[user_name]."】，点击左下角的菜单，展开".$appname."神秘面纱";		
							...
							if ($vv == $keyword) {
								...
								foreach($res as $vvv) {
									...
									if (!empty($vvv['file_url'])) {
										...
										$picurl = $base_url . 'mobile/' . $vvv['file_url'];

									} else {

										$picurl = $base_url . 'mobile/' . 'themes/default/images/logo.gif';

										if (!is_null($GLOBALS['_CFG']['template'])) {

											$picurl = $base_url . 'mobile/themes/' . $GLOBALS['_CFG']['template'] . '/images/logo.gif';
